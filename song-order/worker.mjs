...
const orderRows = (rows, method, randomIterations, preventLargeSkillDifference, preventNoSkillDifference) => {
    ...
    function isValidPlacement(row, index) {
        ...
        if (prevRow) {
            const sharesSongsWithPrevRow =
                row[2] === prevRow[2] || row[3] === prevRow[3] || row[2] === prevRow[3] || row[3] === prevRow[2]
        ...
        if (nextRow) {
            const sharesSongsWithNextRow =
                row[2] === nextRow[2] || row[3] === nextRow[3] || row[2] === nextRow[3] || row[3] === nextRow[2]
    ...
    if (method === 'procedural') {
        ...
        let iterations = 1
        const remainingIndexes = Array.from({ length: orderedRows.length }, (_, i) => i).filter((i) => !orderedRows[i])

        const solutions = []
        let percentNotified = 0
        do {
            if (iterations % 100_000 === 0) console.log('Iteration:', iterations)
            ...
            // Send progress update when it crosses a percentage
            if (Math.floor((iterations / randomIterations) * 100) > percentNotified) {
                percentNotified = Math.floor((iterations / randomIterations) * 100)
                self.postMessage({ iterations: randomIterations / 100 })
            }
        ...
        for (const permutation of permutations) {
            ...
            if (permutation_counter % 10_000_000 === 0) {
                ...
                console.log('Percentage:', ((permutation_counter / totalPermutations) * 100).toFixed(2) + '%')
                console.log(
                    'Time remaining (hours):',
                    (((new Date() - start) / permutation_counter) * (totalPermutations - permutation_counter)) /
                        3600000,
                )
